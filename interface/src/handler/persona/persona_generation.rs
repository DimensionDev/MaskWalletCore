...
use chain_common::api::{
    mw_response::Response, MwResponse, MwResponseError, PersonaGenerationParam,
    PersonaGenerationResp,
};

use crypto::{
    bip39::{CryptoMnemonic, Mnemonic},
    DerivationPath,
};
...
fn generate_persona_inner(param: &PersonaGenerationParam) -> Result<Response, MwResponseError> {
    ...
    let _mnemonic = Mnemonic::new(param.mnemonic.as_str(), "")?;
    let _path = DerivationPath::from_str(param.path.as_str())?;
    let _curve = match &param.curve {
        Some(curve) => Ok(curve),
        None => Err(MwResponseError {
            error_code: "-1".to_string(),
            error_msg: "empty curve".to_string(),
        }),
    }?;
...
mod test {
    ...
    use chain_common::api::curve::Curve;
    use crypto::{Hash, HashEngine, Hmac, HmacEngine, SHA512Hash};
    ...
    fn suit_test() {
        for suilt in vec![PeronaTestSuit::bulk_suit(), PeronaTestSuit::doss_suit()] {
            ...
            let password = "";
            let mnemonic = CryptoMnemonic::from_str(&suilt.mnemonic_str.to_lowercase()).unwrap();
            ...
            engine.input("Bitcoin seed".as_bytes());
            engine.input(&seed);
            // engine.pr

            let sha512hash = SHA512Hash::from_engine(engine);
            let il: Vec<u8> = sha512hash[0..32].into_iter().map(|x| x.clone()).collect();
            let ir: Vec<u8> = sha512hash[32..].into_iter().map(|x| x.clone()).collect();
            ...
            // assert_eq!(suilt.master_key.private_key, il);
            // assert_eq!(suilt.master_key.chaincode, ir);
        }
    }
